    ...
    <section className="noise relative min-h-[calc(100vh-76px)] overflow-hidden px-4 pb-20 pt-16 sm:px-6 lg:px-8 lg:pb-24 lg:pt-24">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.1fr_.9fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }}>
          <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[.04] px-4 py-2 text-xs uppercase tracking-[.22em] text-[#c7d7eb]">
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <Link to="/contact" className="group inline-flex items-center justify-center gap-3 rounded-full bg-[#0052ff] px-7 py-4 text-sm font-bold text-white transition hover:bg-white">
              Start a project <ArrowUpRight className="transition group-hover:rotate-45" size={18} />
            </Link>
            <Link to="/portfolio" className="inline-flex items-center justify-center rounded-full border border-white/15 px-7 py-4 text-sm font-semibold text-white transition hover:border-white/40 hover:bg-white/5">
              Explore our work
            </Link>
          </div>
